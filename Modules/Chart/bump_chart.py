    ...
    for column in bump_df.columns:
        ...
        if column in topics:
            ...
            axs.plot(bump_df.index, bump_df[column], marker=marker, alpha=alpha,
                     markersize=10, label=column, color=color, mec="black")
        else:
            axs.plot(bump_df.index, bump_df[column], marker=marker, alpha=alpha,
                     markersize=10, color=color, mec="black")
    ...
    for year in years:
        if year % 2 == 1:
            ...
            for n in range(len(serie)):
                ...
                text = serie.iloc[n, 1]
                axs.annotate(text, xy=(x, y), ha="left", color="gray", fontsize=fz - 2)
                axs.quiver(x + 0.5, y - 0.4, 0.25, 0, color="gray", scale_units='xy', angles='xy', scale=1.2, width=0.002)
            c += 2

    axs.set_xticks(bump_df.index)
    axs.set_xticklabels(years, fontsize=fz)
    axs.set_yticks(range(0, int(np.nanmax(bump_df.values)) + 2, 5))
    axs.set_yticklabels(range(0, int(np.nanmax(bump_df.values)) + 2, 5), fontsize=fz)
    axs.set_ylim(0, 45)
    axs.set_xlim(2014, 2024.5)
    axs.invert_yaxis()
    axs.set_xlabel("Year", fontsize=fz + 2)
    axs.set_ylabel("Ranking", fontsize=fz + 2)
    axs.grid(which='major', linestyle='--', linewidth='0.5', color='black')
    axs.grid(which='minor', linestyle=':', linewidth='0.5', color='gray')
    axs.legend(title="Keyword", bbox_to_anchor=(1.05, 0.5), loc='center left', fontsize=fz)
    plt.tight_layout()
    st.pyplot(fig)
